...
def spring_exp(url):
    ...
    spring_text.see(END)
    spring_text.insert(END, chars="[%] Checking " + url + " for Spring4Shell RCE CVE-2022-22965.\n")
    spring_text.see(END)
    ...
    if len(vulnerable_hosts) == 0:
        spring_text.insert(END, chars="[•] " + url + " No affected targets were discovered.\n")
        spring_text.see(END)
    else:
...
def run_spring4shell():
    try:
        ...
        urls = get_url()
        for url in urls:
            future = executor.submit(spring_exp,url)
